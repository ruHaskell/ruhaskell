...
Позвольте мне сначала дать кратний обзор обычной реализации
...
Я просто опишу две функции. Первая, `add`, берёт дерево и список (где ранк дерева
...
## Вложенное представлени - первая попытка.
...
Быдет представлен вложенными тройками
...
Все все функции записываются очень просто, за исключением `getMin` и `getMin_`
...
с очевидной семантикой: `const` должен иметь сложноть `O(1)`, но `rev` может
...
разных рангов. Мы строим гипотезу о неком базовом типе обозначающим
...
строить данные этих типов, мы просто использует эти типы, чтобы описать позицию в очереди.
Поэтому мы можем опредлить `Base` и `Succ` как
...
Он включает стрелочный тип (arrow type), который мы не часто встречаем.
...
Для оптимизиронного представления он превращается в 
...
что если бы у нас были экзестенциальные типы, то мы могли бы записать
...
Некоторые реализации на самом деле поддерживают экзестенциальные типы,
но в ограниченных возможностях и могут приводить к потере эффективности
при их использовании...)

Заместо возвращения дочерних элементов и затем применения их к `Nil`, 
...
а не всех детей временных минимумо, которые мы получаем при построении.
Хотя это обозначает, что мы строим гораздо больше thunks вида `(xs Nil)`
при построении результата. И построение этих thunks может вероятно достаточно
дорого.

В 2009 году Addendum Ralf Hinze предложил похожее представление в Разделе 6
...
это разве это не удивительно насколько далеко мы можем зайти используя только
вложенные типы. 
