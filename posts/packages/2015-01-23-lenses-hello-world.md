...
Мы как бы берём действие `set` и, фокусируясь на клубе через клубную линзу `name`, изменяем этим действием имя клуба. Грубо и схематично это можно представить себе так:
...
А правда, друзья мои, состоит в том, что всё вышеизложенное есть не более чем мааааааленькая демонстрация. Существует мощный пакет [lens](http://hackage.haskell.org/package/lens), в котором ~всё уже украдено до нас~ всё уже реализовано очень умными людьми. К рассмотрению возможностей этого пакета мы приступим в следующих статьях.
