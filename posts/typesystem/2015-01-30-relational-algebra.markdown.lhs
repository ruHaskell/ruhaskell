...
кортежей, где у каждого кортежа есть аттрибуты определенного типа. Для представления
...
типом $b \rightarrow a$, обозначающем, что имени `b` соответсвует значение типа `a`.
...
прокси, описывающей тип значения, которые получится после переименования.
...
которая по имени аттрибута достает значение из кортежа
...
условия на соотвествующие аттрибуты.
